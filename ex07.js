...
const checkAir = function (samples, threshold) {
  // Code here!
  //count dirty and clean and return the value where the count is bigger
  //need length of array to calculate portion
  const total = samples.length
  const dirtyCount = samples.reduce((count, test) => {
    return test === "dirty" ? count + 1 : count
  }, 0)

  return dirtyCount / total < threshold ? "Clean" : "Polluted"
};
